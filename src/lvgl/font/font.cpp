...
namespace lvgl::font {
    ...
    const std::shared_ptr<const uint8_t[]> Font::get_glyph_bitmap(uint32_t letter) const {
        return std::shared_ptr<const uint8_t[]>(lv_font_get_glyph_bitmap(this->raw_ptr(), letter));
    }
