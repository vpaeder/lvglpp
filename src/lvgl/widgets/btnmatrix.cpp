...
namespace lvgl::widgets {
    ...
    void ButtonMatrix::set_map(const std::vector<std::string> & map) {
        ...
        // and one cannot assume it's not going to change (also it usually doesn't happen).
    ...
    const std::shared_ptr<const char> ButtonMatrix::get_btn_text(uint16_t btn_id) const {
        return std::shared_ptr<const char>(lv_btnmatrix_get_btn_text(this->raw_ptr(), btn_id));
    }
